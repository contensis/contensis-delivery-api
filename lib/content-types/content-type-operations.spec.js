...
describe('Content Type Operations using client credentials', function () {
    beforeEach(() => {
        specs_utils_spec_1.setDefaultSpyForClientCredentials(global, {
    ...
    it('Get', async () => {
        let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfigForClientCredentials());
        let contentType = await client.contentTypes.get('movie');
        expect(global.fetch).toHaveBeenCalledTimes(2);
        expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
        expect(global.fetch.calls.mostRecent().args).toEqual([
            'http://my-website.com/api/delivery/projects/myProject/contentTypes/movie',
            specs_utils_spec_1.getDefaultFetchRequestForClientCredentials()
        ]);
        expect(contentType).not.toBeNull();
