...
describe('Entry Operations', function () {
    ...
    it('Do Search via the Client API', () => {
        ...
        expect(global.fetch).toHaveBeenCalled();
        let expectedQueryString = utils_1.toQuery(Object.assign({}, query, { orderBy: JSON.stringify(orderBy), where: JSON.stringify(where) }));
    ...
    it('Do Search via the Client API with a link depth', () => {
        ...
        expect(global.fetch).toHaveBeenCalled();
        let expectedQueryString = utils_1.toQuery(Object.assign({}, query, { orderBy: JSON.stringify(orderBy), where: JSON.stringify(where), linkDepth: 99 }));
        expect(global.fetch).toHaveBeenCalledWith(`http://my-website.com/api/delivery/projects/myProject/entries/search${expectedQueryString}`, Object({
            method: 'GET',
            mode: 'cors',
            headers: {
    ...
    it('Do Search via the Client API using the default Query instance', () => {
        ...
        client.entries.search(query);
        expect(global.fetch).toHaveBeenCalled();
        let expectedQueryString = utils_1.toQuery(Object.assign({}, query));
        expect(global.fetch).toHaveBeenCalledWith(`http://my-website.com/api/delivery/projects/myProject/entries/search${expectedQueryString}`, Object({
            method: 'GET',
            headers: {
                'Content-Type': 'application/json; charset=utf-8',
