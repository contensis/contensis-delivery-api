...
describe('Entry Operations', function () {
    ...
    describe('Search entries', () => {
        ...
        it('Do Search - simple', async () => {
            ...
            expect(global.fetch.calls.mostRecent().args).toEqual([
                ...
                specs_utils_spec_1.getDefaultFetchRequestForAccessToken('GET', 'application/json; charset=utf-8')
            ]);
        });
        ...
        it('Do Search with all options', async () => {
            ...
            await client.entries.search(query, 99);
            let expectedQueryString = contensis_core_api_1.toQuery(Object.assign({}, query, { orderBy: JSON.stringify(orderBy), where: JSON.stringify(where), linkDepth: 99, fieldLinkDepths: JSON.stringify(fieldLinkDepths), aggregations: JSON.stringify(aggregations) }));
            expect(global.fetch).toHaveBeenCalled();
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/delivery/projects/myProject/entries/search${expectedQueryString}`,
                specs_utils_spec_1.getDefaultFetchRequestForAccessToken('GET', 'application/json; charset=utf-8')
    ...
    describe('Entry Operations in IE browser', function () {
        ...
        it('Do Search using an object with all options and url = 2083', async () => {
            ...
            await client.entries.search(query, 99);
            let expectedQueryString = contensis_core_api_1.toQuery(Object.assign({}, query, { aggregations: JSON.stringify(aggregations), orderBy: JSON.stringify(orderBy), where: JSON.stringify(where), linkDepth: 99 }));
            expect(global.fetch).toHaveBeenCalled();
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/delivery/projects/myProject/entries/search${expectedQueryString}`,
                specs_utils_spec_1.getDefaultFetchRequestForAccessToken('GET', 'application/json; charset=utf-8')
        ...
        it('Do Search using an object with all options and url > 2083', async () => {
            ...
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/delivery/projects/myProject/entries/search${expectedQueryString}`,
                specs_utils_spec_1.getDefaultFetchRequestForAccessToken('POST', 'application/json; charset=utf-8', false, JSON.stringify(query))
            ]);
        });
