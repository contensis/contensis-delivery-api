...
const defaultFetch = browserGlobal ? browserGlobal.fetch.bind(browserGlobal) : cross_fetch_1.default;
...
class Client {
    ...
    constructor(config = null) {
        ...
        const ipListSource = configIpList != null
            ...
            : (typeof process !== 'undefined'
                ? (_a = process.env) === null || _a === void 0 ? void 0 : _a.API_IP_LIST
                : undefined);
        if (ipListSource && (typeof ipListSource === 'string' || ipListSource.length > 0) && this.clientConfig.rootUrl) {
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(this.fetchFn, ipListSource, this.clientConfig.rootUrl);
    ...
    getDirectIpStatus() {
        if (!this._directIpState)
            return null;
        return {
            current: (0, direct_ip_fetch_1.selectIp)(this._directIpState.ipList, this._directIpState.ipStates),
            ips: this._directIpState.ipList.map(ip => ({
                ip,
                healthy: (0, direct_ip_fetch_1.isHealthy)(this._directIpState.ipStates.get(ip)),
            })),
        };
    }
    destroy() {
