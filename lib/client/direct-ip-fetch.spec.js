...
const direct_ip_fetch_1 = require("./direct-ip-fetch");
...
describe('Direct IP Fetch', () => {
    afterEach(() => {
        (0, direct_ip_fetch_1._resetRegistries)();
    });
    describe('isHealthy', () => {
        it('should return true when failedAt is null', () => {
            const state = { failures: 0, failedAt: null };
            expect((0, direct_ip_fetch_1.isHealthy)(state)).toBe(true);
        });
        it('should return true when failures below threshold', () => {
            const state = { failures: 2, failedAt: Date.now() };
            expect((0, direct_ip_fetch_1.isHealthy)(state)).toBe(true);
        });
        it('should return false when failures >= threshold and within cooldown', () => {
            const now = Date.now();
            const state = { failures: 3, failedAt: now };
            expect((0, direct_ip_fetch_1.isHealthy)(state, now + 1000)).toBe(false);
        });
        it('should return true when failures >= threshold but past cooldown', () => {
            const now = Date.now();
            const state = { failures: 3, failedAt: now - 31000 };
            expect((0, direct_ip_fetch_1.isHealthy)(state, now)).toBe(true);
        });
        it('should return false at exactly cooldown boundary', () => {
            const now = Date.now();
            const state = { failures: 3, failedAt: now - 29999 };
            expect((0, direct_ip_fetch_1.isHealthy)(state, now)).toBe(false);
        });
        it('should return true at exactly cooldown expiry', () => {
            const now = Date.now();
            const state = { failures: 3, failedAt: now - 30000 };
            expect((0, direct_ip_fetch_1.isHealthy)(state, now)).toBe(true);
        });
    });
    describe('recordSuccess', () => {
        it('should reset failures and failedAt', () => {
            const result = (0, direct_ip_fetch_1.recordSuccess)();
    ...
    describe('recordFailure', () => {
        it('should increment failures and set failedAt', () => {
            const state = { failures: 1, failedAt: null };
            const now = 1000000;
            const result = (0, direct_ip_fetch_1.recordFailure)(state, now);
            expect(result.failures).toBe(2);
            expect(result.failedAt).toBe(1000000);
        });
        it('should increment from zero', () => {
            const state = { failures: 0, failedAt: null };
            const result = (0, direct_ip_fetch_1.recordFailure)(state, 500);
    ...
    describe('isInfraFailure', () => {
        it('should return true for 502', () => {
            expect((0, direct_ip_fetch_1.isInfraFailure)(502)).toBe(true);
        });
        it('should return true for 503', () => {
            expect((0, direct_ip_fetch_1.isInfraFailure)(503)).toBe(true);
        });
        it('should return true for 504', () => {
            expect((0, direct_ip_fetch_1.isInfraFailure)(504)).toBe(true);
        });
        it('should return false for 200', () => {
            expect((0, direct_ip_fetch_1.isInfraFailure)(200)).toBe(false);
        });
        it('should return false for 404', () => {
            expect((0, direct_ip_fetch_1.isInfraFailure)(404)).toBe(false);
        });
        it('should return false for 500', () => {
            expect((0, direct_ip_fetch_1.isInfraFailure)(500)).toBe(false);
        });
        it('should return false for 501', () => {
            expect((0, direct_ip_fetch_1.isInfraFailure)(501)).toBe(false);
        });
    });
    describe('selectIp', () => {
        it('should select the first healthy IP', () => {
            ...
            expect((0, direct_ip_fetch_1.selectIp)(ipList, states)).toBe('10.0.0.1');
        });
        it('should skip unhealthy IPs', () => {
            ...
            states.set('10.0.0.2', { failures: 0, failedAt: null });
            states.set('10.0.0.3', { failures: 0, failedAt: null });
            expect((0, direct_ip_fetch_1.selectIp)(ipList, states, undefined, now + 1000)).toBe('10.0.0.2');
        });
        it('should exclude specified IP', () => {
            ...
            states.set('10.0.0.1', { failures: 0, failedAt: null });
            states.set('10.0.0.2', { failures: 0, failedAt: null });
            expect((0, direct_ip_fetch_1.selectIp)(ipList, states, '10.0.0.1')).toBe('10.0.0.2');
        });
        it('should return null when all IPs are unhealthy', () => {
            ...
            expect((0, direct_ip_fetch_1.selectIp)(ipList, states, undefined, now + 1000)).toBeNull();
        });
        it('should select IP past cooldown for retry', () => {
            const now = Date.now();
            const ipList = ['10.0.0.1', '10.0.0.2'];
            const states = new Map();
            states.set('10.0.0.1', { failures: 3, failedAt: now - 31000 });
            states.set('10.0.0.2', { failures: 3, failedAt: now });
            expect((0, direct_ip_fetch_1.selectIp)(ipList, states, undefined, now)).toBe('10.0.0.1');
        });
        it('should return null when no IPs have state', () => {
            const ipList = ['10.0.0.1'];
            const states = new Map();
            expect((0, direct_ip_fetch_1.selectIp)(ipList, states)).toBeNull();
        });
    });
    describe('validateIps', () => {
        it('should accept valid IPv4 addresses', () => {
            const result = (0, direct_ip_fetch_1.validateIps)(['10.0.0.1', '192.168.1.1'], mockNetModule);
            expect(result).toEqual(['10.0.0.1', '192.168.1.1']);
        });
        it('should reject non-IP values', () => {
            spyOn(console, 'warn');
            const result = (0, direct_ip_fetch_1.validateIps)(['not-an-ip', '10.0.0.1'], mockNetModule);
            expect(result).toEqual(['10.0.0.1']);
            expect(console.warn).toHaveBeenCalled();
        });
        it('should reject cloud metadata IP', () => {
            spyOn(console, 'warn');
            const result = (0, direct_ip_fetch_1.validateIps)(['169.254.169.254', '10.0.0.1'], mockNetModule);
            expect(result).toEqual(['10.0.0.1']);
            expect(console.warn).toHaveBeenCalledWith(jasmine.stringContaining('cloud metadata'));
        });
        it('should skip empty entries', () => {
            const result = (0, direct_ip_fetch_1.validateIps)(['', '  ', '10.0.0.1'], mockNetModule);
            expect(result).toEqual(['10.0.0.1']);
        });
        ...
        it('should cap at 10 IPs', () => {
            ...
            const result = (0, direct_ip_fetch_1.validateIps)(ips, mockNetModule);
            expect(result.length).toBe(10);
            expect(console.warn).toHaveBeenCalledWith(jasmine.stringContaining('capping at 10'));
        });
        it('should return empty array when no valid IPs', () => {
            spyOn(console, 'warn');
            const result = (0, direct_ip_fetch_1.validateIps)(['not-ip', 'also-not'], mockNetModule);
            expect(result).toEqual([]);
        });
        it('should return all IPs as invalid when netModule is null', () => {
            spyOn(console, 'warn');
            const result = (0, direct_ip_fetch_1.validateIps)(['10.0.0.1'], null);
            expect(result).toEqual([]);
        });
    });
    describe('parseIpListEnvVar', () => {
        it('should parse valid env var value', () => {
            const result = (0, direct_ip_fetch_1.parseIpListEnvVar)('cms.example.com|10.0.0.1,10.0.0.2', 'https://cms.example.com/api', mockNetModule);
            expect(result).toEqual({
        ...
        it('should return null for empty value', () => {
            expect((0, direct_ip_fetch_1.parseIpListEnvVar)('', 'https://cms.example.com', mockNetModule)).toBeNull();
        });
        it('should return null for missing rootUrl', () => {
            expect((0, direct_ip_fetch_1.parseIpListEnvVar)('host|10.0.0.1', '', mockNetModule)).toBeNull();
        });
        it('should return null for missing pipe separator', () => {
            spyOn(console, 'warn');
            expect((0, direct_ip_fetch_1.parseIpListEnvVar)('10.0.0.1', 'https://cms.example.com', mockNetModule)).toBeNull();
            expect(console.warn).toHaveBeenCalledWith(jasmine.stringContaining('invalid format'));
        });
        it('should return null for hostname mismatch', () => {
            spyOn(console, 'warn');
            const result = (0, direct_ip_fetch_1.parseIpListEnvVar)('other.example.com|10.0.0.1', 'https://cms.example.com', mockNetModule);
            expect(result).toBeNull();
            expect(console.warn).toHaveBeenCalledWith(jasmine.stringContaining('does not match'));
        });
        it('should return null for invalid rootUrl', () => {
            spyOn(console, 'warn');
            const result = (0, direct_ip_fetch_1.parseIpListEnvVar)('host|10.0.0.1', 'not-a-url', mockNetModule);
            expect(result).toBeNull();
        });
        it('should return null when all IPs are invalid', () => {
            spyOn(console, 'warn');
            const result = (0, direct_ip_fetch_1.parseIpListEnvVar)('cms.example.com|not-an-ip', 'https://cms.example.com', mockNetModule);
            expect(result).toBeNull();
        });
        it('should trim hostname and IPs', () => {
            const result = (0, direct_ip_fetch_1.parseIpListEnvVar)(' cms.example.com | 10.0.0.1 , 10.0.0.2 ', 'https://cms.example.com', mockNetModule);
            expect(result).toEqual({
                hostname: 'cms.example.com',
                ips: ['10.0.0.1', '10.0.0.2']
            });
    ...
    describe('createDirectIpFetch', () => {
        ...
        it('should return inner fetch unchanged when ipListSource is empty string', () => {
            ...
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(innerFetch, '', 'https://cms.example.com', mockDeps);
        ...
        it('should return inner fetch unchanged when hostname does not match', () => {
            spyOn(console, 'warn');
            const innerFetch = createMockFetch();
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(innerFetch, 'other.example.com|10.0.0.1', 'https://cms.example.com', mockDeps);
            expect(result.fetch).toBe(innerFetch);
            expect(result.state).toBeNull();
        });
        it('should create wrapped fetch for valid env var string', () => {
            const innerFetch = createMockFetch([{ status: 200, ok: true }]);
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(innerFetch, 'cms.example.com|10.0.0.1', 'https://cms.example.com', mockDeps);
        ...
        it('should create wrapped fetch for Config.ipList array', () => {
            const innerFetch = createMockFetch([{ status: 200, ok: true }]);
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(innerFetch, ['10.0.0.1', '10.0.0.2'], 'https://cms.example.com', mockDeps);
            expect(result.fetch).not.toBe(innerFetch);
            expect(result.state).not.toBeNull();
        ...
        it('should share state between instances with same key', () => {
            ...
            const result1 = (0, direct_ip_fetch_1.createDirectIpFetch)(fetch1, 'cms.example.com|10.0.0.1', 'https://cms.example.com', mockDeps);
            const result2 = (0, direct_ip_fetch_1.createDirectIpFetch)(fetch2, 'cms.example.com|10.0.0.1', 'https://cms.example.com', mockDeps);
        ...
        it('should reject hostname with control characters via validateHostname', () => {
            ...
            spyOn(console, 'warn');
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(innerFetch, ['10.0.0.1'], 'not-a-valid-url\x00', mockDeps);
            expect(result.fetch).toBe(innerFetch);
            expect(result.state).toBeNull();
        });
        it('should return inner fetch unchanged when rootUrl is empty', () => {
            const innerFetch = createMockFetch();
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(innerFetch, ['10.0.0.1'], '', mockDeps);
            expect(result.fetch).toBe(innerFetch);
        });
    });
    describe('directIpFetch (wrapped function)', () => {
        ...
        it('should rewrite URL to use IP and set Host header', async () => {
            ...
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(innerFetch, ['10.0.0.1'], 'https://cms.example.com', mockDeps);
            // Mark IP as healthy
        ...
        it('should fall back to CDN when all IPs are unhealthy', async () => {
            ...
            expect(console.warn).toHaveBeenCalledWith(jasmine.stringContaining('falling back to CDN'));
        ...
        it('should handle URL with port correctly', async () => {
            ...
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(innerFetch, ['10.0.0.1'], 'https://cms.example.com:8443', mockDeps);
            result.state.ipStates.set('10.0.0.1', { failures: 0, failedAt: null });
    ...
    describe('Config.ipList override', () => {
        ...
        it('should validate Config.ipList IPs the same as env var IPs', () => {
            spyOn(console, 'warn');
            const innerFetch = createMockFetch();
            const result = (0, direct_ip_fetch_1.createDirectIpFetch)(innerFetch, ['not-an-ip', '169.254.169.254'], 'https://cms.example.com', mockDeps);
            expect(result.fetch).toBe(innerFetch);
            expect(result.state).toBeNull();
        });
    });
