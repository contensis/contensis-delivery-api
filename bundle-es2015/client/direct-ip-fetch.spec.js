import { isHealthy, recordSuccess, recordFailure, isInfraFailure, selectIp, validateIps, parseIpListEnvVar, createDirectIpFetch, _resetRegistries, } from './direct-ip-fetch';
...
describe('Direct IP Fetch', () => {
    ...
    describe('parseIpListEnvVar', () => {
        it('should parse valid env var value', () => {
            const result = parseIpListEnvVar('cms.example.com|10.0.0.1,10.0.0.2', 'https://cms.example.com/api', mockNetModule);
            expect(result).toEqual({
        ...
        it('should return null for hostname mismatch', () => {
            spyOn(console, 'warn');
            const result = parseIpListEnvVar('other.example.com|10.0.0.1', 'https://cms.example.com', mockNetModule);
        ...
        it('should return null when all IPs are invalid', () => {
            spyOn(console, 'warn');
            const result = parseIpListEnvVar('cms.example.com|not-an-ip', 'https://cms.example.com', mockNetModule);
            expect(result).toBeNull();
        });
        it('should trim hostname and IPs', () => {
            const result = parseIpListEnvVar(' cms.example.com | 10.0.0.1 , 10.0.0.2 ', 'https://cms.example.com', mockNetModule);
            expect(result).toEqual({
                hostname: 'cms.example.com',
                ips: ['10.0.0.1', '10.0.0.2']
            });
    ...
    describe('createDirectIpFetch', () => {
        ...
        it('should return inner fetch unchanged when hostname does not match', () => {
            spyOn(console, 'warn');
            const innerFetch = createMockFetch();
            const result = createDirectIpFetch(innerFetch, 'other.example.com|10.0.0.1', 'https://cms.example.com', mockDeps);
            expect(result.fetch).toBe(innerFetch);
            expect(result.state).toBeNull();
        });
        it('should create wrapped fetch for valid env var string', () => {
            const innerFetch = createMockFetch([{ status: 200, ok: true }]);
            const result = createDirectIpFetch(innerFetch, 'cms.example.com|10.0.0.1', 'https://cms.example.com', mockDeps);
        ...
        it('should create wrapped fetch for Config.ipList array', () => {
            const innerFetch = createMockFetch([{ status: 200, ok: true }]);
            const result = createDirectIpFetch(innerFetch, ['10.0.0.1', '10.0.0.2'], 'https://cms.example.com', mockDeps);
            expect(result.fetch).not.toBe(innerFetch);
            expect(result.state).not.toBeNull();
        ...
        it('should share state between instances with same key', () => {
            ...
            const result1 = createDirectIpFetch(fetch1, 'cms.example.com|10.0.0.1', 'https://cms.example.com', mockDeps);
            const result2 = createDirectIpFetch(fetch2, 'cms.example.com|10.0.0.1', 'https://cms.example.com', mockDeps);
        ...
        it('should reject hostname with control characters via validateHostname', () => {
            ...
            spyOn(console, 'warn');
            const result = createDirectIpFetch(innerFetch, ['10.0.0.1'], 'not-a-valid-url\x00', mockDeps);
            expect(result.fetch).toBe(innerFetch);
            expect(result.state).toBeNull();
        });
    ...
    describe('directIpFetch (wrapped function)', () => {
        ...
        it('should rewrite URL to use IP and set Host header', async () => {
            ...
            const result = createDirectIpFetch(innerFetch, ['10.0.0.1'], 'https://cms.example.com', mockDeps);
            // Mark IP as healthy
        ...
        it('should handle URL with port correctly', async () => {
            ...
            const result = createDirectIpFetch(innerFetch, ['10.0.0.1'], 'https://cms.example.com:8443', mockDeps);
            result.state.ipStates.set('10.0.0.1', { failures: 0, failedAt: null });
    ...
    describe('Config.ipList override', () => {
        ...
        it('should validate Config.ipList IPs the same as env var IPs', () => {
            spyOn(console, 'warn');
            const innerFetch = createMockFetch();
            const result = createDirectIpFetch(innerFetch, ['not-an-ip', '169.254.169.254'], 'https://cms.example.com', mockDeps);
            expect(result.fetch).toBe(innerFetch);
            expect(result.state).toBeNull();
        });
    });
